# few shot example은 많으면 많을수록 llm의 응답 정확도가 높아진다.
# 하지만 많은 example을 사용하면 token 사용량이 늘어나게 되고 이는 비용의 증가를 초래한다.
# 최소 3개 정도의 example이라면 어느 정도의 정확도를 보여주지만
# llm 응답을 확인하여 정확도를 더 높여야 하는 경우라면 example을 하나씩 추가하고 응답 상태를 확인하도록 한다.
